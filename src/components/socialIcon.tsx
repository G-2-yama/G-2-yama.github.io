import React from 'react';

interface IconBase {
  lightTheme?: string;
  darkTheme?: string;
}

interface SvgIconData extends IconBase {
  path: string;
  imageUrl?: never;
}

interface ImageIconData extends IconBase {
  imageUrl: string;
  path?: never;
}

export type IconDataEntry = SvgIconData | ImageIconData;

// SVGのパス定義をまとめたマップ
export const IconData: { [key: string]: IconDataEntry } = {
// svgの場合は https://simpleicons.org/ を使用してアイコンを取得すると楽
// 色はテーマごとに指定できる
// 画像の場合は配置したパスを入力
  twitter: {
    path: 'M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 24h6.993zm-2.837 3.299-.929-1.329L3.076 1.56h3.182l5.965 8.532.929 1.329 7.754 11.09h-3.182z',
    lightTheme: '#000000',
    darkTheme: '#ffffff',
  },
  github: {
    path: 'M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12',
    lightTheme: '#181717',
    darkTheme: '#ffffff',
  },
  youtube: {
    path: 'M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z',
    lightTheme: '#FF0033',
    darkTheme: '#FFFFFF',
  },
  pixiv: {
    path: 'M4.94 0A4.953 4.953 0 0 0 0 4.94v14.12A4.953 4.953 0 0 0 4.94 24h14.12A4.953 4.953 0 0 0 24 19.06c-.014 1.355 0-14.12 0-14.12A4.953 4.953 0 0 0 19.06 0Zm1.783 5.465h.904a.37.37 0 0 1 .31.17l.752 1.17a6.172 6.172 0 0 1 10.01 4.834 6.172 6.172 0 0 1-9.394 5.265v2.016a.37.37 0 0 1-.37.367H6.724a.37.37 0 0 1-.37-.367V5.834a.37.37 0 0 1 .37-.37m5.804 2.951a3.222 3.222 0 1 0-.002 6.443 3.222 3.222 0 0 0 .002-6.443',
    lightTheme: '#0096FA',
    darkTheme: '#ffffff',
  },
  homepage: {
    path: 'M1.992.034C1.217.166.602.841.416 1.769.388 1.901.36 21.858.388 23.895c0 .181 0 .181.923-.593.497-.416 1.328-1.128 1.801-1.516.34-.28.626-.543.637-.576.011-.039.022-1.148.017-2.466L3.76 16.35h1.516c.829 0 1.52.022 1.537.044.017.021.028.538.022 1.142 0 .61.011 1.104.022 1.104.017 0 .643-.522.725-.604.017-.017.401-.335.851-.714.923-.77.917-.764 1.373-1.148.28-.236.34-.264.412-.203.072.066 2.878 2.421 3.592 3.02a1239.37 1239.37 0 0 1 3.932 3.306c.003.003 2.02 1.74 2.076 1.702.021-.01.038-3.333.038-7.38-.006-6.574.005-7.365.082-7.381.044-.011.895-.017 1.884-.017h1.801l-.022-3.761c-.005-2.07-.033-3.817-.05-3.877-.142-.495-.51-1.022-.883-1.28-.473-.318.164-.302-10.566-.302-5.442-.005-9.99.011-10.11.033Z',
    lightTheme: '#000000',
    darkTheme: '#ffffff',
  },
};

// アイコンをラップする共通コンポーネント
interface IconWrapperProps {
  name: keyof typeof IconData;
  className?: string;
}

export const IconWrapper: React.FC<IconWrapperProps> = ({ name, className }) => {
  const data = IconData[name];
  
  if (!data) {
    // 定義されていないアイコン名が渡された場合
    return <span className={className}>❓</span>;
  }

  if (data.imageUrl) {
    return (
      <img
        src={data.imageUrl}
        alt={`${name} icon`}
        className={className}
        loading='lazy'
      />
    )
  }

  return (
    <svg 
      role="img" 
      viewBox="0 0 24 24" 
      xmlns="http://www.w3.org/2000/svg" 
      className={className}
      style={{
        '--icon-light-color': data.lightTheme,
        '--icon-dark-color': data.darkTheme,
        color: 'var(--icon-light-color)',
      } as React.CSSProperties}
    >
      <path d={data.path} fill="currentColor" />
    </svg>
  );
};